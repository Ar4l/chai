...
Closing the lid normally forces sleep regardless of power assertions; the only supported
override is `pmset disablesleep`, which requires administrator privileges. With this preference
enabled, Chai runs `pmset disablesleep 1` while a keep-awake session is active and
`pmset disablesleep 0` when it ends (deactivation, timer expiry, or quitting Chai). To gain
privileges, Chai first tries a passwordless `sudo -n`, then falls back to a macOS password
prompt.
...
Enabled by default, matching upstream behavior. Turn it **off** and Chai pauses its keep-awake
session while on battery power and resumes it on AC — so a "Forever" session can't silently
drain the battery. While paused, the menu bar icon shows the empty mug and the menu reads
